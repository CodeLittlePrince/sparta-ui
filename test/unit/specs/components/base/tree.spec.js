...
describe('Tree', () => {
  ...
  const getTreeVm = (props, options) => {
    return createVue(Object.assign({
      template: `
        <sp-tree ref="tree" :data="data" ${props}></sp-tree>
        `,
  ...
  it('create', () => {
    ...
    expect(document.querySelector('.sp-tree')).to.exist
    expect(document.querySelectorAll('.sp-tree > .sp-tree-node').length).to.equal(3)
    expect(document.querySelectorAll('.sp-tree .sp-tree-node').length).to.equal(ALL_NODE_COUNT)
  ...
  it('click node', done => {
    ...
    const firstNode = vm.$el.querySelector('.sp-tree-node__content')
    firstNode.click()
    expect(vm.clickedNode.label).to.equal('一级 1')
    setTimeout(() => {
      expect(vm.$el.querySelector('.sp-tree-node').classList.contains('is-expanded')).to.true
      firstNode.click()
      setTimeout(() => {
        expect(vm.$el.querySelector('.sp-tree-node').classList.contains('is-expanded')).to.false
        done()
      }, DELAY)
    }, DELAY)
  })
  ...
  it('emptyText', (done) => {
    ...
    vm.data = []
    vm.$nextTick(() => {
      expect(vm.$el.querySelectorAll('.sp-tree__empty-block').length).to.equal(1)
      done()
    })
  })
  ...
  it('highlight current', done => {
    ...
    const firstNode = document.querySelector('.sp-tree-node')
    firstNode.click()
    vm.$nextTick(() => {
  ...
  it('current-node-key', () => {
    ...
    const currentNodeLabel = document.querySelector('.is-current .sp-tree-node__label').textContent
  ...
  it('defaultExpandAll', () => {
    vm = getTreeVm(':props="defaultProps" default-expand-all')
    expect(vm.$el.querySelectorAll('.sp-tree-node.is-expanded').length).to.equal(ALL_NODE_COUNT)
  })
  ...
  it('defaultExpandedKeys', () => {
    ...
    expect(vm.$el.querySelectorAll('.sp-tree-node.is-expanded').length).to.equal(2)
  })
  ...
  it('defaultExpandedKeys set', (done) => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelectorAll('.sp-tree-node.is-expanded').length).to.equal(1)
      done()
    }, 50)
  })
  ...
  it('filter-node-method', (done) => {
    ...
    setTimeout(() => {
      expect(tree.$el.querySelectorAll('.sp-tree-node.is-hidden').length).to.equal(3)
      done()
    }, 100)
  })
  ...
  it('autoExpandParent = true', () => {
    ...
    expect(vm.$el.querySelectorAll('.sp-tree-node.is-expanded').length).to.equal(3)
  })
  ...
  it('autoExpandParent = false', (done) => {
    ...
    expect(vm.$el.querySelectorAll('.sp-tree-node.is-expanded').length).to.equal(0)
    const firstNode = document.querySelector('.sp-tree-node__content')
    firstNode.querySelector('.sp-tree-node__expand-icon').click()
    setTimeout(() => {
      expect(document.querySelectorAll('.sp-tree-node.is-expanded').length).to.equal(2)
      done()
    }, 100)
  })
  ...
  it('defaultCheckedKeys & check-strictly = false', () => {
    ...
    expect(vm.$el.querySelectorAll('.sp-checkbox.is--checked').length).to.equal(3)
  })
  ...
  it('defaultCheckedKeys & check-strictly', () => {
    ...
    expect(vm.$el.querySelectorAll('.sp-checkbox.is--checked').length).to.equal(1)
  })

  it('show checkbox', done => {
    vm = getTreeVm(':props="defaultProps" show-checkbox')
    const tree = vm.$children[0]
    const secondNode = document.querySelectorAll('.sp-tree-node__content')[1]
    const nodeCheckbox = secondNode.querySelector('.sp-checkbox')
    ...
    setTimeout(() => {
      ...
      secondNode.querySelector('.sp-tree-node__expand-icon').click()
      setTimeout(() => {
        const firstLeaf = secondNode.nextElementSibling.querySelector('.sp-tree-node__content')
        const leafCheckbox = firstLeaf.querySelector('.sp-checkbox')
        leafCheckbox.click()
        setTimeout(() => {
  ...
  it('set disabled checkbox', done => {
    ...
    const node = document.querySelectorAll('.sp-tree-node__content')[2]
    const nodeCheckbox = node.querySelector('.sp-checkbox input')
    vm.$nextTick(() => {
  ...
  it('check strictly', (done) => {
    ...
    const tree = vm.$children[0]
    const secondNode = document.querySelectorAll('.sp-tree-node__content')[3]
    const nodeCheckbox = secondNode.querySelector('.sp-checkbox')
    nodeCheckbox.click()
    expect(tree.getCheckedNodes().length).to.equal(1)
  ...
  it('render content', () => {
        ...
        renderContent(h, node) {
          return (
            <span class="custom-content">
              <sp-button>{node.node.label}</sp-button>
            </span>
          )
        }
      }
    ...
    const button = firstNode.querySelector('.custom-content .sp-button')
  ...
  it('scoped slot', () => {
    ...
    const button = firstNode.querySelector('.sp-button')
  ...
  it('load node', done => {
    ...
    const nodes = document.querySelectorAll('.sp-tree-node__content')
    expect(nodes.length).to.equal(2)
    nodes[0].click()
    setTimeout(() => {
      expect(document.querySelectorAll('.sp-tree-node__content').length).to.equal(4)
      done()
    }, 100)
  })
  ...
  it('lazy defaultChecked', done => {
    ...
    setTimeout(() => {
      ...
      const child = document.querySelectorAll('.sp-tree-node__content')[1]
  ...
  it('accordion', done => {
    ...
    const firstNode = vm.$el.querySelector('.sp-tree-node__content')
    const secondNode = vm.$el.querySelector('.sp-tree-node:nth-child(2) .sp-tree-node__content')
    firstNode.click()
    setTimeout(() => {
      expect(vm.$el.querySelector('.sp-tree-node').classList.contains('is-expanded')).to.true
      secondNode.click()
  ...
  it('updateKeyChildren', (done) => {
    ...
    const node = document.querySelectorAll('.sp-tree-node__content')[2]
    const label = node.querySelector('.sp-tree-node__label')
    vm.$nextTick(() => {
  ...
  it('update multi tree data', async () => {
    const vm = createVue({
      template: `
        <div>
          <sp-tree ref="tree1" :data="data" node-key="id" :props="defaultProps"></sp-tree>
          <sp-tree ref="tree2" :data="data" node-key="id" :props="defaultProps"></sp-tree>
        </div>
        `,

...
import CascaderPanel from 'sparta/components/base/cascader-panel'
...
const getValidOptions = (el, menuIndex) => getMenus(el)[menuIndex].querySelectorAll('.sp-cascader-node[tabindex="-1"]')
const getLabel = el => el.querySelector('.sp-cascader-node__label').textContent
...
describe('CascaderPanel', () => {
    ...
    vm = createVue({
      template: `
        <sp-cascader-panel
          ref="panel"
          v-model="value"
          :options="options"></sp-cascader-panel>
      `,
      data() {
        return {
          value: [],
  ...
  it('with default value', async () => {
    ...
    // expect(getOptions(el, 2)[0].querySelector('.sp-icon-check')).to.exist
  })
  ...
  it('multiple mode', async () => {
    ...
    const el = vm.$el
    const checkbox = getOptions(el, 0)[0].querySelector('.sp-checkbox__wrap')
    expect(checkbox).to.exist
    expect(checkbox.querySelector('.sp-checkbox').className).to.not.includes('is--checked')
    checkbox.querySelector('input').click()

    await waitImmediate()
    expect(checkbox.querySelector('.sp-checkbox').className).to.includes('is--checked')
  ...
  it('multiple mode with disabled default value', async () => {
    ...
    await waitImmediate()
    expect(checkbox).to.exist
    // expect(checkbox.querySelector('.sp-checkbox').className).to.includes('is--indeterminate')
    checkbox.querySelector('input').click()
  ...
  it('check strictly in single mode', async () => {
    ...
    const el = vm.$el
    const radio = getOptions(el, 0)[0].querySelector('.sp-radio__wrap')
    ...
    expect(getOptions(el, 0)[1].querySelector('.sp-radio__wrap').className).to.includes('is--disabled')
  })
  ...
  it('check strictly in multiple mode', async () => {
    ...
    const el = vm.$el
    const checkbox = getOptions(el, 0)[0].querySelector('.sp-checkbox')
    ...
    getOptions(el, 0)[0].click()
    await waitImmediate()
    expect(getOptions(el, 1)[0].querySelector('.sp-checkbox').className).to.not.includes('is--checked')
    getOptions(el, 1)[0].querySelector('input').click()
    await waitImmediate()
    expect(vm.value).to.deep.equal(['zhejiang', 'hangzhou'])
    expect(getOptions(el, 0)[1].querySelector('.sp-checkbox').className).to.includes('is--disabled')
  })
  ...
  it('dynamic loading', async () => {
    ...
    firstOption.click()
    await waitImmediate()
    expect(firstOption.querySelector('i').className).to.includes('sp-icon-loading')
    await wait(500)

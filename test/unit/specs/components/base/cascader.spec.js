...
import Cascader from 'sparta/components/base/cascader'
...
const getMenus = el => el.querySelectorAll('.sp-cascader-menu')
const getOptions = (el, menuIndex) => getMenus(el)[menuIndex].querySelectorAll('.sp-cascader-node')
const selectedValue = ['zhejiang', 'hangzhou', 'xihu']
const getCloseButton = el => el.querySelectorAll('i.sp-icon-close')
...
describe('Cascader', () => {
  ...
  it('expand and check', async () => {
    ...
    // expect(checkedOption.querySelector('i.sp-icon-check')).to.exist
  ...
  it('clearable', async () => {
    ...
    await waitImmediate()
    const closeBtn = vm.$el.querySelector('i.sp-input__icon')
    ...
    vm = createVue({
      template: `
        <sp-cascader
          v-model="value"
          :options="options"
          :show-all-levels="false"></sp-cascader>
      `,
      data() {
        return {
          value: selectedValue,
  ...
  it('multiple mode', async () => {
    ...
    getOptions(document.body, 0)[0].querySelector('.sp-checkbox input').click()
    await waitImmediate()
    expect(vm.value.length).to.equal(3)
    expect(getCloseButton(vm.$el).length).to.equal(3)

    const tags = vm.$el.querySelectorAll('.sp-tag')
    const closeBtn = tags[0].querySelector('.sp-icon-close')
  ...
  it('clearable in multiple mode', async () => {
    ...
    await waitImmediate()
    expect(getOptions(document.body, 0)[0].querySelector('.sp-checkbox.is--checked')).to.exist
    triggerEvent(vm.$el, 'mouseenter')
    await waitImmediate()
    const closeBtn = vm.$el.querySelector('i.sp-input__icon')
    expect(closeBtn).to.exist
  ...
  it('collapse tags', async () => {
    ...
    expect(tags[0].querySelector('.sp-icon-close')).to.exist
    expect(tags[1].querySelector('.sp-icon-close')).to.be.null
    tags[0].querySelector('.sp-icon-close').click()
    expect(tags[1].textContent.trim()).to.equal('+ 3')
    await waitImmediate()
    expect(vm.value.length).to.equal(3)
    vm.$el.querySelector('.sp-tag .sp-icon-close').click()
    await waitImmediate()
    vm.$el.querySelector('.sp-tag .sp-icon-close').click()
    await waitImmediate()
    expect(vm.$el.querySelector('.sp-tag')).to.exist
    // disabled tag can not be closed
    expect(vm.$el.querySelector('.sp-tag').querySelector('.sp-icon-close')).to.be.null
  })
  ...
  it('filterable', async () => {
    ...
    await wait(300)
    expect(body.querySelector('.sp-cascader__suggestion-list')).to.exist
    expect(body.querySelectorAll('.sp-cascader__suggestion-item').length).to.equal(3)
    body.querySelectorAll('.sp-cascader__suggestion-item')[0].click()
    await waitImmediate()
    expect(vm.value).to.deep.equal(selectedValue)
  })
  ...
  it('filter method', async () => {
    ...
    triggerEvent(input, 'input')
    await wait(300)
    expect(body.querySelector('.sp-cascader__suggestion-item').textContent.trim()).to.equal('Zhejiang / Hangzhou / West Lake')
  })
})

...
describe('Switch', () => {
  ...
  it('create', () => {
    ...
    const core = vm.$el.querySelector('.sp-switch__core')
    expect(core.style.backgroundColor).to.equal('rgb(255, 0, 0)')
    expect(core.style.width).to.equal('100px')
    expect(vm.$el.querySelector('.sp-switch__labsp--left').querySelector('span').textContent).to.equal('off')
  })

  it('switch with icons', () => {
    vm = createTest(Switch, {
      activeIconClass: 'sp-icon-check',
      inactiveIconClass: 'sp-icon-close'
    })

    const icon = vm.$el.querySelector('.sp-switch__labsp--left').querySelector('i')
    expect(icon.classList.contains('sp-icon-close')).to.true
  })
  ...
  it('disabled switch should not respond to user click', done => {
    vm = createVue({
      template: `
        <div>
          <sp-switch disabled v-model="value"></sp-switch>
        </div>
      `,
    ...
    vm.$el.querySelector('.sp-switch__core').click()
    Vue.nextTick(() => {
  ...
  it('expand switch value', done => {
    vm = createVue({
      template: `
        <div>
          <sp-switch v-model="value" :active-value="onValue" :inactive-value="offValue"></sp-switch>
        </div>
      `,
      data() {
        return {
  ...
  it('value is the single source of truth', done => {
    vm = createVue({
      template: `
        <div>
          <sp-switch :value="true"></sp-switch>
        </div>
      `
    }, true)
  ...
  it('sets checkbox value', done => {
    vm = createVue({
      template: `
        <div>
          <sp-switch v-model="value"></sp-switch>
        </div>
      `,
      data() {
        return {

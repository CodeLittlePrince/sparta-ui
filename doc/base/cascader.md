...
<template>
  <div class="block">
    ...
    <sp-cascader
      v-model="value"
      :options="options"
      @change="handleChange"></sp-cascader>
  </div>
  <div class="block">
...
```vue
<template>
  <sp-cascader :options="options"></sp-cascader>
</template>
<script>
  export default {
    data() {
...
:::demo 通过 `clearable` 设置输入框可清空
```vue
<template>
  <sp-cascader :options="options" clearable></sp-cascader>
</template>
<script>
  export default {
    data() {
...
```vue
<template>
  <sp-cascader :options="options" :show-all-levels="false"></sp-cascader>
</template>
<script>
  export default {
    data() {
...
```vue
<template>
  <sp-cascader :props="props"></sp-cascader>
</template>
<script>
  let id = 0;
...
```vue
<template>
  <sp-cascader-panel :options="options"></sp-cascader-panel>
</template>
...
<script>
  ...
  export default {
    data() {
      return {
        value: '',
        value1: ['zujian', 'form', 'input-number'],
        props: {
          lazy: true,
          lazyLoad (node, resolve) {
            const { level } = node;
